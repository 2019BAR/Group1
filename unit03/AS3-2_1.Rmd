...
nrow(IBM)*ncol(IBM)
nrow(GE)*ncol(GE)
nrow(ProcterGamble)*ncol(ProcterGamble)
nrow(CocaCola)*ncol(CocaCola)
nrow(Boeing)*ncol(Boeing)
#每一個公司資料框的row*col就可以知道有幾個觀察值
#
dim(IBM)
dim(GE)
dim(ProcterGamble)
...
IBM_ts = as.POSIXct(IBM$Date, format="%m/%d/%y")
GE_ts = as.POSIXct(GE$Date, format="%m/%d/%y")
ProcterGamble_ts = as.POSIXct(ProcterGamble$Date, format="%m/%d/%y")
CocaCola_ts = as.POSIXct(CocaCola$Date, format="%m/%d/%y")
Boeing_ts = as.POSIXct(Boeing$Date, format="%m/%d/%y")
#table(format(GE_ts,'%y')) %>% order %>% head
...
```{r}
IBM$Date[480]
#table(format(GE_ts,'%y')) %>% order %>% tail
#在資料集中最晚的年份是?
#
```
...
#x = seq(from = as.Date("2000-03-01"), to = as.Date("2000-04-01"),by = 1)
#set.seed(123)
#y = sample(0:1000,size = 50 , replace = TRUE)
#line_IBM = data.frame(x = IBM$StockPrice, y= IBM$Date)
#ggplot(line_IBM,aes(X = x,y= y)) + geom_line() + scale_x_date(date_labels =  "%m.%d")
...
#1997年10月，全球股市崩盤是由亞洲經濟危機引發的。 比較1997年9月至1997年11月，哪些公司的股價下跌趨勢？ （選擇所有符合條件的。）
#
```
...
```{r}
#在12月和1月份，每家公司的平均庫存在一個月內較高而在另一個月較低。 哪個月股價下跌？
#
```
